...
public class LeftHitbox extends Collision{
	
	public LeftHitbox(float x, float y, int length) {
		super(x,y,x,y+length);
	}
	
	public boolean collidingWith(Collision c, float xVel, float yVel) { //madeline running left into wall
		float slope = yVel/xVel;
		float midx = (c.x+c.xEnd)/2;
		float midy = (c.y+c.yEnd)/2;
		
		if (c instanceof RightHitbox) {
